    ...
    db.all("PRAGMA table_info(suppliers)", (err, rows) => {
        ...
        columnsToAdd.forEach(column => {
            if (!existingColumns.includes(column.name)) {
                console.log(`Adding column ${column.name} to suppliers...`);
                db.run(`ALTER TABLE suppliers ADD COLUMN ${column.name} ${column.type}`, (err) => {
                    if (err) {
                        console.error(`Error adding column ${column.name}:`, err.message);
                    } else {
                        console.log(`✅ Column ${column.name} added to suppliers successfully.`);
                    }
                });
            } else {
                console.log(`Column ${column.name} already exists in suppliers.`);
            }
        });
    });
});
